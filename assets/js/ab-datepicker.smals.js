...
(function(factory){
  ...
  var datepickerButton = [
    ...
    // '</a>'
    '<div class="input-group-appendQuery">',
      '<button class="btn btn-primary datepicker-button" type="button"><i class="fa fa-calendar" aria-hidden="true"></i> <span class="sr-only">Select date</span></button>',
    '</div>'
  ];
  var datepickerCalendar = [
    '<div class="datepicker-calendar" id="datepicker-calendar-CALENDARID" aria-hidden="false">',
    ' <div class="datepicker-month-wrap">',
    '   <div class="datepicker-month-fast-next pull-right" role="button" aria-labelledby="datepicker-bn-fast-next-label-CALENDARID" tabindex="0"><span class="fa fa-angle-double-right"></span></div>',// edit syv smals (glyphicon > fa)
    '   <div class="datepicker-month-next pull-right" role="button" aria-labelledby="datepicker-bn-next-label-CALENDARID" tabindex="0"><span class="fa fa-angle-right"></span></div>',
    '   <div class="datepicker-month-fast-prev pull-left" role="button" aria-labelledby="datepicker-bn-fast-prev-label-CALENDARID" tabindex="0"><span class="fa fa-angle-double-left"></span></div>',
    '   <div class="datepicker-month-prev pull-left" role="button" aria-labelledby="datepicker-bn-prev-label-CALENDARID" tabindex="0"><span class="fa fa-angle-left"></span></div>',
  ...
  var Datepicker = function (target, options) {
    ...
    this.$button.find('span').text(this.options.buttonTitle);
  ...
  Datepicker.prototype.populateDaysCalendar = function() {
    ...
    if (this.options.previous !== null && this.options.previous.val() !== '') {
      previousDate = this.options.previous.datepicker('getDate');
    }
    if (this.options.next !== null && this.options.next.val() !== '') {
      nextDate = this.options.next.datepicker('getDate');
    }
    ...
    for (curDay = 1; curDay <= numDays; curDay++) {
      ...
      var currentDate = new Date(this.curYear, this.curMonth, this.date, 0, 0, 0, 0);
      //var curDayClass = curDay == this.date && this.month == this.curMonth && this.year == this.curYear ? ' curDay' : '';
      var curDayClass = date.getTime() == currentDate.getTime() ? ' curDay' : '';
      
      // add extra classes (today, start,) /syv smals
      var today = new Date();
      var todaylongdate = this.formatDate(today, this.options.titleFormat)
      var todayClass = todaylongdate == longdate ? ' today' : '';

      var startClass = '' , endClass = '', rangeClass = '';
      if(previousDate) {
        startClass = previousDate.getTime() == date.getTime() ? ' range-start' : '';
        rangeClass = previousDate.getTime() < date.getTime() && date.getTime() < currentDate.getTime() ? ' range' : '';
      }
      if(nextDate) {
        endClass = nextDate.getTime() == date.getTime() ? ' range-end' : '';
        rangeClass = currentDate.getTime() < date.getTime() && date.getTime() < nextDate.getTime() ? ' range' : '';
      }

      var extraClasses = curDayClass + todayClass + startClass + endClass + rangeClass;


      if (isYearDisabled || isMonthDisabled) {
        gridCells += '\t\t<td id="cell' + curDay + '-' + this.id + '" class="day unselectable'+extraClasses+'"';
        ...
        gridCells += '\t\t<td id="cell' + curDay + '-' + this.id + '" class="day selectable'+extraClasses+'"';
      }
